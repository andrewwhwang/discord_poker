from exceptions import (CardCreationError,
...
class Card(object):
    ...
    * suit - the suit of the card [0-3]
    ...
    def str_sym(self):
        symbol = ""
        if self.suits[self.suit] == 'c':
            symbol = "♣"
        elif self.suits[self.suit] == 'd':
            symbol = "♦"
        elif self.suits[self.suit] == 'h':
            symbol = "♥"
        elif self.suits[self.suit] == 's':
            symbol = "♠"
        return '%s%s' % (self.ranks[self.rank], symbol)
